                           ...
                           prrte_buffer_t *buf)
{
    ...
    fprintf(stderr, "apps = %d, num_noes = %d\n", jdata->num_apps, jdata->map->num_nodes);
    for (i=0; i < jdata->num_apps; i++) {
        ...
        if (NULL != (app = (prrte_app_context_t*)prrte_pointer_array_get_item(jdata->apps, i))) {
            for (j=0; j < jdata->map->num_nodes; j++) {
                ...
                ppn = 0;
                fprintf(stderr, "num_procs=%d, procs->size = %d\n", nptr->procs->size, nptr->num_procs);
                for (k=0; k < nptr->num_procs; k++) {
                         ...
                         prrte_buffer_t *buf)
{
    ...
    /* reset any flags */
    for (m=0; m < jdata->map->num_nodes; m++) {
